...
The scanner detects the host operating system at runtime (`win32`, `darwin`, `linux`) and
resolves agent locations against that system's native home and application-data roots.
Windows uses `%APPDATA%` and `%LOCALAPPDATA%`, macOS uses `~/Library`, and Linux uses
`XDG_CONFIG_HOME` / `XDG_DATA_HOME` with `~/.config` and `~/.local/share` fallbacks. Paths
belonging to another operating system are **skipped rather than searched under the wrong
home directory**, so a macOS `Library/` template never becomes `C:\Users\me\Library`.

For VS Code-family editors the scanner checks the traditional `User/workspaceStorage` and
`User/globalStorage` locations **and** the newer sibling `agentSessionData/*/session.db`
used by recent VS Code builds, through the `<vscode-data>` placeholder.

The scan catalog exposes the resolved platform so the UI can say where it is looking:

```js
catalog.platform
// { id: "win32", name: "windows", roots: { home, appData, localAppData, config, data } }
```

From the `electron-app` directory, pass backend CLI arguments after the extra separator:
